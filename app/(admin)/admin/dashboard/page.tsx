...
export default function AdminDashboardPage() {
  ...
  const handleSeedData = async () => {
    setIsSeeding(true);
    try {
      const res = await fetch("/api/v1/tasks/seed", { method: "POST" });
      const json = await res.json();
      if (json.success) {
        await fetchAllData();
      }
    } catch (err) {
      console.error("Failed to seed workspace data:", err);
    } finally {
      setIsSeeding(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {tasks.length === 0 && !isLoading && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/25 text-xs font-bold hover:bg-amber-500/25 transition disabled:opacity-50 cursor-pointer shadow-sm"
            >
              <Sparkles className={`w-3.5 h-3.5 ${isSeeding ? "animate-spin" : ""}`} />
              {isSeeding ? "Loading Tasks..." : "✨ Load Sample Data"}
            </button>
          )}
      ...
      {/* ── Empty Workspace Starter Banner ── */}
      {tasks.length === 0 && !isLoading && (
        <div className="relative overflow-hidden p-6 rounded-2xl bg-gradient-to-r from-primary/10 via-violet-500/10 to-indigo-500/10 border border-primary/25 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-3.5">
              <div className="p-2.5 rounded-xl bg-primary text-white shadow-md shadow-primary/25 flex-shrink-0">
                <Sparkles className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                  Welcome to your new TASQ-ONE Workspace!
                </h3>
                <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 max-w-xl">
                  Your workspace has 0 tasks right now. Click below to instantly load realistic starter tasks, Kanban columns, and productivity metrics in 1 click.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                disabled={isSeeding}
                onClick={handleSeedData}
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-white text-xs font-bold shadow-md shadow-primary/25 transition-all hover:scale-[1.02] disabled:opacity-50 cursor-pointer"
              >
                <Sparkles className={`w-4 h-4 ${isSeeding ? "animate-spin" : ""}`} />
                <span>{isSeeding ? "Seeding Tasks..." : "✨ Load Sample Workspace"}</span>
              </button>
              <button
                type="button"
                onClick={() => setIsTaskModalOpen(true)}
                className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:text-primary transition shadow-sm cursor-pointer"
              >
